...
if pgrep wf-recorder > /dev/null; then
    ...
    if [[ "$1" == "--sound" ]]; then
        wf-recorder --pixel-format yuv420p -f $filename -t --geometry "$(slurp)" --audio="$(getaudiooutput)" & disown
        ...
        wf-recorder --pixel-format yuv420p -f $filename -t --geometry "$(slurp)" & disown
    fi
fi
