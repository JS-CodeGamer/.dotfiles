...
-- Normalize JSONC into strict JSON before decoding.
local function strip_jsonc(text)
	...
	while i <= #text do
		...
		if in_string then
			...
			elseif ch == "/" and next_ch == "*" then
				i = i + 2
				while i <= #text - 1 and not (text:sub(i, i) == "*" and text:sub(i + 1, i + 1) == "/") do
					i = i + 1
				end
				i = i + 2
			else
...
local function parse_tools_config()
	...
	if not ok then
		vim.notify("Failed to read tools config", vim.log.levels.ERROR)
		return {}
	end
	...
	local ok_decode, parsed_data = pcall(vim.json.decode, content)
...
local function get_filetype_to_server()
	if cache.filetype_to_server then
		return cache.filetype_to_server
	end
...
local function get_filetype_to_formatter()
	if cache.filetype_to_formatter then
		return cache.filetype_to_formatter
	end
...
local function get_filetype_to_linter()
	if cache.filetype_to_linter then
		return cache.filetype_to_linter
	end
...
local function get_lsp_actions_autoexec()
	if cache.lsp_actions_autoexec then
